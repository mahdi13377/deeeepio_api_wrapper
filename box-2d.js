...
b2Settings.b2_massUnitsPerKilogram = 10.0;
...
b2Settings.b2_velocityThreshold = (0 * b2Settings.b2_lengthUnitsPerMeter) / b2Settings.b2_timeUnitsPerSecond;
...
Object.extend(b2PolyShape.prototype, {
	...
	Support: function (dX, dY, out) {
		...
		out.Set(
			this.m_position.x + (this.m_R.col1.x * this.m_coreVertices[bestIndex].x + this.m_R.col2.x * this.m_coreVertices[bestIndex].y),
...
b2BodyDef.prototype = {
	initialize: function (fixed) {
		...
		this.rotation = 0.0;
		this.linearVelocity = fixed ? new b2Vec2(0, 0) : new b2Vec2((Math.random() - 0.5) * 100, (Math.random() - 0.5) * 100);
...
Object.extend(b2GearJoint.prototype, {
	GetAnchor1: function () {
		...
		return new b2Vec2(
			this.m_body1.m_position.x + (tMat.col1.x * this.m_localAnchor1.x + tMat.col2.x * this.m_localAnchor1.y),
	...
	GetAnchor2: function () {
		...
		return new b2Vec2(
			this.m_body2.m_position.x + (tMat.col1.x * this.m_localAnchor2.x + tMat.col2.x * this.m_localAnchor2.y),
...
// DOM STUFF
var delta = [0, 0];
const stageElementDimensions = document.getElementById("penguinGravity").getBoundingClientRect();
var stage = [stageElementDimensions.width, stageElementDimensions.height, stageElementDimensions.width, stageElementDimensions.height];
...
function init() {
	...
	worldAABB.maxVertex.Set(stageElementDimensions.width + 200, stageElementDimensions.height + 200);
	...
	for (var i = 0; i < elements.length; i++) {
		...
		bodies[i] = createBox(
			...
			properties[i][2] / 2,
			properties[i][3] / 2,
			element.style.transform.replace("rotate(", "").replace("deg)", "") * (Math.PI / 180),
			false
		);
...
// .. BOX2D UTILS
function createBox(world, x, y, width, height, rotation, fixed, element) {
	if (typeof fixed == "undefined") fixed = true;
	var boxSd = new b2BoxDef();
	if (!fixed) boxSd.density = 100.0;
	boxSd.extents.Set(width, height);
	var boxBd = new b2BodyDef(fixed);
	boxBd.AddShape(boxSd);
	boxBd.position.Set(x, y);
	boxBd.rotation = rotation || 0;
	boxBd.userData = { element: element };
	return world.CreateBody(boxBd);
}
function mouseDrag() {
	relMouse = {
		x: mouse.x - document.body.getBoundingClientRect().x,
		y: mouse.y - document.body.getBoundingClientRect().y
	};
	// mouse press
	if (isMouseDown && !mouseJoint) {
		...
		if (body) {
			...
			md.body2 = body;
			md.target.Set(relMouse.x, relMouse.y);
			md.maxForce = 15000.0 * body.m_mass;
	...
	// mouse move
	if (mouseJoint) {
		var p2 = new b2Vec2(relMouse.x, relMouse.y);
...
function getBodyAtMouse() {
	...
	mousePVec.Set(relMouse.x, relMouse.y);
	var aabb = new b2AABB();
	aabb.minVertex.Set(relMouse.x - 1, relMouse.y - 1);
	aabb.maxVertex.Set(relMouse.x + 1, relMouse.y + 1);
...
function getBrowserDimensions() {
	var changed = false;
	if (stage[0] != stageElementDimensions.width) {
		delta[0] = (stageElementDimensions.width - stage[0]) * 50;
		stage[0] = stageElementDimensions.width;
		changed = true;
	}
	if (stage[1] != stageElementDimensions.height) {
		delta[1] = (stageElementDimensions.height - stage[1]) * 50;
		stage[1] = stageElementDimensions.height;
		changed = true;
	}
	if (stage[2] != stageElementDimensions.width) {
		stage[2] = stageElementDimensions.width;
		changed = true;
	}
	if (stage[3] != stageElementDimensions.height) {
		stage[3] = stageElementDimensions.height;
		changed = true;
	}
	return changed;
}
